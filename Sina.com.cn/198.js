	...
	$(function(){
		var dataids = [];
		console.log($('#pl_feed_conf_206').find('[name="cmnt_count"]').length)
		$('#pl_feed_conf_206').find('[name="cmnt_count"]').each(function(){
		...
		function cmtNum(data){
			...
			for(var o in numberlist){
				...
				if(str == 0){
					$('#pl_feed_conf_206').find("[cmnt_id='"+o+"']").html("评论");
				}else{
					$('#pl_feed_conf_206').find("[cmnt_id='"+o+"']").html(str);
				}
			}
		}
		$.ajax({
